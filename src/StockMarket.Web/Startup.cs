        ...
        {
            {
                services.AddDbContext<StockMarketContext>(options =>
                    options.UseSqlServer(Configuration.GetConnectionString("StockMarketDbConnectionString"))
                );
                services.AddScoped<StockMarketBusinessSevice>();
            ...
            {
                c.SwaggerDoc("v1", new Info { Title = "Stock Market API", Version = "v1" });
            });
        }
